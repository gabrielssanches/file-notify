							...
							gpointer user_data)
{
	...
	case G_IO_STATUS_AGAIN:
		return TRUE;
	default:
		g_warning("Reading from inotify channel failed");
		inotify->watch = 0;
		return FALSE;
	}
...
static int create_watch(const char *path, struct connman_inotify *inotify)
{
	...
	g_message("Add directory watch for %s", path);
	...
	if (inotify->wd < 0) {
		g_warning("Creation of %s watch failed", path);
		close(fd);
		return -EIO;
	}
	...
	if (!inotify->channel) {
		g_warning("Creation of inotify channel failed");
